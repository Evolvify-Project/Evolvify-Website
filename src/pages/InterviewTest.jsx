...
const InterviewTestPage = () => {
  ...
  const [topicTimer, setTopicTimer] = useState(QUESTION_DURATION);
  ...
  // UseEffect to handle the question timer and synchronize countdown
  useEffect(() => {
    if (!recording) return;
    ...
    const interval = setInterval(() => {
      setTopicTimer((prev) => {
        if (prev <= 0) {
          ...
          if (nextQuestion < interviewQuestions.length) {
            ...
            return QUESTION_DURATION; // Reset timer for the next question
          } else {
            // Reached the last question, stop the recording
      ...
      // Calculate total remaining time based on current question and topicTimer
      setCountdown(() => {
        const remainingQuestions = totalQuestions - currentQuestion - 1;
        const remainingTime =
          remainingQuestions * QUESTION_DURATION + topicTimer;
        return remainingTime > 0 ? remainingTime : 0;
      });
    }, 1000);

    return () => clearInterval(interval); // Cleanup on unmount or when recording stops
  }, [recording, currentQuestion, mediaRecorder, topicTimer, totalQuestions]);
  ...
  const startInterview = async () => {
    ...
    setTopicTimer(QUESTION_DURATION);
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        setCurrentQuestion(0);
        setTopicTimer(QUESTION_DURATION);
        showMessage("Interview finished. Please submit or cancel.");
        if (chunks.length > 0) {
          ...
          setVideoDuration(totalQuestions * QUESTION_DURATION);
      ...
      setCountdown(totalQuestions * QUESTION_DURATION);
    } catch (err) {
            ...
            <div className="mt-4 bg-white rounded-2xl p-4 shadow-lg border border-gray-100 transition-all duration-500 transform animate-fadeIn">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center">
                  ...
                  <h3 className="font-semibold text-lg text-gray-800">
                    Current Question (Question {currentQuestion + 1} of{" "}
                    {totalQuestions})
                  </h3>
                </div>
              ...
              <div className="mt-3 w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${(topicTimer / QUESTION_DURATION) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
